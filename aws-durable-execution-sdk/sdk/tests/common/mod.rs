...
impl MockDurableServiceClient {
    ...
    pub fn with_checkpoint_response_with_callback(self, token: &str, callback_id: &str) -> Self {
            ...
            .push(Ok(CheckpointResponse {
                ...
                new_execution_state: Some(NewExecutionState {
                    operations: vec![{
                        ...
                        let mut op =
                            Operation::new("__CALLBACK_PLACEHOLDER__", OperationType::Callback);
            ...
            if let Some(ref mut new_state) = response.new_execution_state {
                for op in &mut new_state.operations {
                    if op.operation_id == "__CALLBACK_PLACEHOLDER__" {
                        ...
                        if let Some(callback_op) = operations
                            .iter()
                            .find(|o| o.operation_type == OperationType::Callback)
                        {
...
pub fn create_context_operation(
    id: &str,
...
pub fn create_execution_operation(
    id: &str,
    status: OperationStatus,
    input: Option<&str>,
) -> Operation {
...
pub fn error_object_strategy() -> impl Strategy<Value = ErrorObject> {
    (non_empty_string_strategy(), non_empty_string_strategy())
        .prop_map(|(error_type, error_message)| ErrorObject::new(error_type, error_message))
}
...
pub fn optional_error_strategy() -> impl Strategy<Value = Option<ErrorObject>> {
    prop_oneof![Just(None), error_object_strategy().prop_map(Some),]
}
        ...
        .prop_map(|(id, action, op_type, result, error, parent_id, name)| {
            let mut update = match action {
                ...
                OperationAction::Fail => {
                    let err = error
                        .clone()
                        .unwrap_or_else(|| ErrorObject::new("TestError", "Test error message"));
...
pub fn operation_update_batch_strategy(
    max_size: usize,
) -> impl Strategy<Value = Vec<OperationUpdate>> {
