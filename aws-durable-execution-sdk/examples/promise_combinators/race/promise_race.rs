...
use aws_durable_execution_sdk::{race, durable_execution, DurableError, DurableContext, MapConfig, CompletionConfig};
...
#[durable_execution]
pub async fn handler_with_macro(_event: serde_json::Value, ctx: DurableContext) -> Result<String, DurableError> {
...
#[durable_execution]
pub async fn handler_with_timeout(_event: serde_json::Value, ctx: DurableContext) -> Result<String, DurableError> {
    ...
    let result = race!(ctx,
        async move {
            ...
            ctx1.step(|_| Ok("operation completed".to_string()), None).await
        },
        async move {
            // Timeout - returns error to signal timeout
            ctx2.step(|_| Err::<String, _>("operation timed out".into()), None).await
        },
    ).await;
...
pub async fn handler(_event: serde_json::Value, ctx: DurableContext) -> Result<String, DurableError> {
    ...
    let operations: Vec<String> = vec!["fast".to_string(), "medium".to_string(), "slow".to_string()];
    ...
    let batch_result = ctx.map(
        ...
        |child_ctx: DurableContext, name: String, _index: usize| {
            Box::pin(async move {
                child_ctx.step(|_| Ok(format!("{} result", name)), None).await
            })
        },
        Some(config),
    ).await?;
