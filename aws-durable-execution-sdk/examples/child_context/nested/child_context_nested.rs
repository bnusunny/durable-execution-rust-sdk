...
#[durable_execution]
pub async fn handler(_event: serde_json::Value, ctx: DurableContext) -> Result<Vec<NestedResult>, DurableError> {
    let mut results = Vec::new();

    // Level 1 child context
    let level1_result = ctx
        .run_in_child_context_named("level_1", |child_ctx| {
            Box::pin(async move {
                let result: NestedResult = child_ctx
                    .step(|_| Ok(NestedResult {
                        level: 1,
                ...
                let level2_result = child_ctx
                    .run_in_child_context_named("level_2", |nested_ctx| {
                        Box::pin(async move {
                            nested_ctx
                                .step(|_| Ok(NestedResult {
                                    level: 2,
