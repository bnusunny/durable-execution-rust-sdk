...
use aws_durable_execution_sdk::{
    durable_execution, Duration, DurableError,
};
...
#[durable_execution]
pub async fn handler(_event: serde_json::Value, ctx: DurableContext) -> Result<ServiceResults, DurableError> {
    ...
    let payment = ctx
        .run_in_child_context_named("process_payment", |child_ctx| {
            Box::pin(async move {
                // Payment includes a delay
                child_ctx.wait(Duration::from_seconds(1), Some("payment_delay")).await?;
                child_ctx
    ...
    let shipping = ctx
        .run_in_child_context_named("calculate_shipping", |child_ctx| {
            Box::pin(async move {
                child_ctx
