...
async fn test_replay_multiple_completed_steps_returns_cached_results() {
    ...
    assert!(
        calls.is_empty(),
        "No checkpoint calls should be made during replay"
    );
}
...
async fn test_replay_preserves_result_order() {
    ...
    for i in 0..5 {
        ...
        let result: Result<ReplayResult<i32>, DurableError> =
            check_replay(&checkpoint, OperationType::Step, &op_id, TEST_EXECUTION_ARN);
        assert!(result.is_ok());
        match result.unwrap() {
...
async fn test_replay_pending_step_returns_in_progress() {
    ...
    assert!(
        checkpoint2.is_pending(),
        "Operation should be in pending status"
    );
...
async fn test_replay_pending_wait_signals_suspension() {
    ...
    assert!(
        ...
        "Pending wait should not be terminal"
    );
    assert!(
        !checkpoint.is_succeeded(),
        "Pending wait should not be succeeded"
    );
}
...
async fn test_replay_pending_callback_signals_suspension() {
    ...
    assert!(
        !checkpoint.is_terminal(),
        "Pending callback should not be terminal"
    );
...
async fn test_replay_mixed_operation_types() {
    ...
    assert!(
        callback_checkpoint.is_succeeded(),
        "Callback should be succeeded"
    );
    assert_eq!(
        callback_checkpoint.operation_type(),
    ...
    match result.unwrap_err() {
        DurableError::NonDeterministic {
            ...
            assert!(
                ...
                "Message should mention found type"
            );
        }
...
async fn test_replay_detects_all_type_mismatches() {
    ...
    for (checkpointed_type, expected_type) in test_cases {
        ...
        let result: Result<ReplayResult<String>, DurableError> =
            check_replay(&checkpoint, expected_type, "test-op", TEST_EXECUTION_ARN);
...
async fn test_replay_preserves_deep_nesting() {
    ...
    let result: Result<ReplayResult<String>, DurableError> =
        check_replay(&step_cp, OperationType::Step, "step-1", TEST_EXECUTION_ARN);
...
async fn test_replay_mode_transitions_after_all_replayed() {
    ...
    assert!(
        state.is_new(),
        "Should transition to new mode after all replayed"
    );
}
...
async fn test_replay_non_existent_operation_returns_not_found() {
    ...
    assert!(
        !checkpoint.is_existent(),
        "Non-existent operation should not exist"
    );
