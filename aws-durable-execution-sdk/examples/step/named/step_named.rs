...
#[durable_execution]
pub async fn handler(_event: serde_json::Value, ctx: DurableContext) -> Result<Vec<ProcessingResult>, DurableError> {
    // Step with explicit name
    let first: ProcessingResult = ctx
        .step_named("fetch_data", |_step_ctx| {
            Ok(ProcessingResult {
    ...
    let second: ProcessingResult = ctx
        .step_named("process_data", |_step_ctx| {
            Ok(ProcessingResult {
