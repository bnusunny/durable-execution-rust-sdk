...
// For more complex workflows with parallel processing:
#[durable_execution]
async fn process_batch_orders(event: BatchEvent, ctx: DurableContext) -> Result<BatchResult<OrderResult>, DurableError> {
    // Process multiple orders in parallel with max 5 concurrent
    ...
    ctx.map(
        event.orders,
        |child_ctx, order, _index| {
            // Each order processed in its own child context
            child_ctx.step(|_| process_single_order(order), None)
        },
        Some(config),
    ).await
}

// Example with callbacks for external system integration:
#[durable_execution]
async fn approval_workflow(event: ApprovalRequest, ctx: DurableContext) -> Result<ApprovalResult, DurableError> {
    // Create a callback and wait for external approval
    let callback = ctx.create_callback::<ApprovalResponse>(Some(CallbackConfig {
        timeout: Duration::from_hours(24),
        ..Default::default()
    })).await?;

    // Send callback_id to external system (e.g., via SNS, SQS, or API)
    notify_approver(&callback.callback_id, &event.request_id).await?;

    // Suspend execution until callback is received
    // External system calls SendDurableExecutionCallbackSuccess with the callback_id
...
Each `ctx.step()` call is automatically checkpointed. If Lambda recycles the execution environment or the function waits for a callback, execution resumes from the last checkpoint without re-executing completed steps.

## Overview

This document describes the technical design for implementing the AWS Durable Execution SDK for the Lambda Rust Runtime. The SDK enables Rust developers to build reliable, long-running workflows in AWS Lambda with automatic checkpointing, replay, and state management.
...
The SDK follows a layered architecture similar to the Python implementation:
...
impl DurableContext {
    ...
    pub async fn run_in_child_context<T, F>(&self, func: F, config: Option<ChildConfig>) -> Result<T, DurableError>
    where
        T: Serialize + DeserializeOwned,
...
*For any* Duration value, constructing from seconds/minutes/hours/days SHALL produce the correct total seconds, and wait operations SHALL reject durations less than 1 second.
...
pub enum DurableError {
    ...
    Callback { message: String, callback_id: Option<String> },
