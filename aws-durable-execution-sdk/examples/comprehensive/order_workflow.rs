...
use aws_durable_execution_sdk::{
    durable_execution, CallbackConfig, CompletionConfig, Duration, DurableContext,
    DurableError, MapConfig,
};
...
pub async fn handler(event: OrderEvent, ctx: DurableContext) -> Result<OrderResult, DurableError> {
    ...
    let is_valid: bool = ctx
        .step_named("validate_order", |_| {
            Ok(!event.items.is_empty())
        }, None)
        .await?;
    ...
    let _inventory_check: String = ctx
        .run_in_child_context_named("check_inventory", |child_ctx| {
            Box::pin(async move {
                child_ctx
    ...
    let _customer_check: String = ctx
        .run_in_child_context_named("check_customer", |child_ctx| {
            Box::pin(async move {
                child_ctx
    ...
    let processed_items = ctx
        .map(
            ...
            |child_ctx: DurableContext, item: OrderItem, index: usize| {
                Box::pin(async move {
                    child_ctx
                        .step_named(&format!("process_item_{}", index), |_| {
                            Ok(item.price_cents * item.quantity as u64)
                        }, None)
                        .await
                })
            },
    ...
    ctx.wait(Duration::from_seconds(2), Some("processing_delay")).await?;

    // Step 6: Finalize order
    let _finalized: bool = ctx
        .step_named("finalize_order", |_| Ok(true), None)
        .await?;
