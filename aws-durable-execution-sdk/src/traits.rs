...
mod tests {
    ...
    fn test_step_fn_closure() {
        ...
        let closure = |_ctx: StepContext| -> Result<i32, Box<dyn std::error::Error + Send + Sync>> {
            Ok(42)
        };
    ...
    fn test_step_fn_with_capture() {
        ...
        let closure = move |_ctx: StepContext| -> Result<i32, Box<dyn std::error::Error + Send + Sync>> {
            Ok(captured_value)
        };
        assert_step_fn(closure);
    }
    ...
    fn test_step_fn_returning_struct() {
        ...
        let closure = |_ctx: StepContext| -> Result<TestStruct, Box<dyn std::error::Error + Send + Sync>> {
            Ok(TestStruct {
                id: "test".to_string(),
                value: 1,
            })
    ...
    fn test_step_fn_borrowing_closure() {
        ...
        let closure = |_ctx: StepContext| -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
