...
pub async fn payment_with_error_handling(
    ...
    let result: Result<PaymentConfirmation, DurableError> = ctx
        .wait_for_callback(
            move |callback_id| {
                let payment_id = payment_id_for_closure;
                ...
                async move {
                    ...
                    println!(
                        "Notifying payment processor: {} with callback {}",
    ...
    // Handle the result
    match result {
        Ok(confirmation) => Ok(PaymentResult {
...
pub async fn multi_stage_order_workflow(
    ...
    if event.total > 1000.0 {
        ...
        let approval: ApprovalResponse = ctx
            .wait_for_callback(
                move |callback_id| {
    ...
    let shipping: ShippingConfirmation = ctx
        .wait_for_callback(
            move |callback_id| {
                let oid = oid_for_closure;
        ...
        .wait_for_callback(
            move |callback_id| {
                ...
                async move {
                    ...
                    println!(
                        "Notifying processor: payment={}, callback={}",
                        payment_id, callback_id
                    );
