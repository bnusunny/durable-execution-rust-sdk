...
#[durable_execution]
pub async fn handler(_event: serde_json::Value, ctx: DurableContext) -> Result<Vec<ProcessedItem>, DurableError> {
    ...
    let results = ctx
        .map(
            ...
            |child_ctx, item: String, index: usize| {
                Box::pin(async move {
                    child_ctx
                        .step_named(&format!("process_{}", index), |_| {
                            Ok(ProcessedItem {
                                id: item.clone(),
    ...
    results.get_results().map(|refs| refs.into_iter().cloned().collect())
}
