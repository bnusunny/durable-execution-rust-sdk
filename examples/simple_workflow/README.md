...
```bash
simple_workflow$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
simple_workflow$ sam local start-api
simple_workflow$ curl http://localhost:3000/
```
...
```bash
simple_workflow$ sam logs -n HelloWorldFunction --stack-name simple_workflow --tail
```
