...
use aws_durable_execution_sdk::{any, durable_execution, DurableError, DurableContext, MapConfig, CompletionConfig};
...
pub async fn handler_with_macro(_event: serde_json::Value, ctx: DurableContext) -> Result<String, DurableError> {
    ...
    let result = any!(ctx,
        async move {
            ...
            ctx1.step(|_| Err::<String, _>("primary unavailable".into()), None).await
        },
        async move {
            // Secondary source succeeds
            ctx2.step(|_| Ok("data from secondary".to_string()), None).await
        },
        async move {
            // Fallback source (may not be called if secondary succeeds)
            ctx3.step(|_| Ok("data from fallback".to_string()), None).await
        },
    ).await?;
...
pub async fn handler(_event: serde_json::Value, ctx: DurableContext) -> Result<String, DurableError> {
    ...
    let batch_result = ctx.map(
        ...
        |child_ctx: DurableContext, (name, should_succeed): (String, bool), _index: usize| {
            Box::pin(async move {
                if should_succeed {
                    child_ctx.step(|_| Ok(format!("{} succeeded", name)), None).await
                } else {
