...
async fn test_wait_for_callback_replay_behavior() {
    ...
    let has_context = calls.iter().any(|call| {
        call.operations.iter().any(|op| op.operation_type == OperationType::Context)
    });
    assert!(has_context, "Should have context checkpoint for child context");
}
...
async fn test_wait_for_callback_creates_correct_checkpoint_structure() {
    ...
    assert!(
        first_call.operations.iter().any(|op| op.operation_type == OperationType::Callback),
